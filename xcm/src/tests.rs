...
fn dmp() {
    ...
    ParaA::execute_with(|| {
        assert!(system_contains_event!(
            parachain,
            System(frame_system::Event::Remarked { .. })
        ));
    });
}
...
fn reserve_transfer() {
    ...
    Relay::execute_with(|| {
        ...
        assert!(
            attempted_emitted,
            "Expected XcmPallet::Attempted event emitted"
        );
...
fn reserve_transfer_with_error() {
    ...
    subscriber::with_default(subscriber, || {
        ...
        Relay::execute_with(|| {
            ...
            assert!(
                result.is_err(),
                "Expected an error due to invalid destination"
            );
...
fn query_holding() {
    ...
    ParaA::execute_with(|| {
        let received = ReceivedDmp::<parachain::Runtime>::get();
        assert_eq!(received.len(), 1);
        let instructions = &received[0].0;
        // First instruction should be QueryResponse
        assert!(matches!(
            &instructions[0],
            QueryResponse {
                query_id,
