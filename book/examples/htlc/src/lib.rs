...
pub mod pallet {
    ...
    pub trait Config: frame_system::Config {
        ...
        /// Escrow movement
        type Escrow: EscrowTrust<Self::AccountId, Self::AssetId, Self::Balance>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn open_htlc(
            ...
            // Lock into escrow
            T::Escrow::escrow_lock(asset, &maker, amount).map_err(|_| Error::<T>::Arithmetic)?;
        ...
        pub fn redeem_with_secret(
            ...
            T::Escrow::escrow_release(rec.asset, to, rec.amount)
        ...
        pub fn refund(origin: OriginFor<T>, htlc_id: u64) -> DispatchResult {
            ...
            T::Escrow::escrow_refund(rec.asset, &rec.maker, rec.amount)
                .map_err(|_| Error::<T>::Arithmetic)?;
    ...
    impl<T: Config> BridgeHtlc<T::AccountId, T::AssetId, T::Balance> for Pallet<T> {
        ...
        fn open_htlc(
            ...
            // Lock into escrow
            T::Escrow::escrow_lock(asset, maker, amount)?;
